    ...
    methods: {
      ...
      _normalizeSongs(list) {
        ...
        list.forEach((songlist) => {
          this._getDiscSongVkey(songlist.songmid)
          if (songlist.songid && songlist.albumid) {
            ret.push(createDiscSong(songlist, this.vkey))
          }
        })
        console.log(ret)
        return ret
      },
      _getDiscSongVkey(item) {
        getDiscSongVkey(item).then((res) => {
          if (res.code === ERR_OK) {
            this.vkey = res.req_0.data.midurlinfo[0].vkey;
          }
        })
      },
      _checkMore(data) {
